...
<xsl:stylesheet version="3.0"
   ...
   <xsl:variable name="tx_implementation">Texas DIR Control Standard</xsl:variable>
   <xsl:variable name="tamus_implementation">Texas A&amp;M University System Control Standard</xsl:variable>
   <xsl:variable name="tamux_implementation">Texas A&amp;M University - Xample Implementation Statement</xsl:variable>

   <xsl:variable name="tx_baseline">Texas DIR Baseline</xsl:variable>
   <xsl:variable name="tamus_baseline">Texas A&amp;M University System Baseline</xsl:variable>
   <xsl:variable name="tamux_baseline">Texas A&amp;M University - Xample Baseline</xsl:variable>

   <xsl:variable name="tx_privacy_baseline">Texas DIR Privacy Baseline</xsl:variable>
   <xsl:variable name="tamus_privacy_baseline">Texas A&amp;M University System Privacy Baseline</xsl:variable>
   <xsl:variable name="tamux_privacy_baseline">Texas A&amp;M University - Xample Privacy Baseline</xsl:variable>

   <xsl:variable name="tx_new_requirement">Texas DIR New Required Control</xsl:variable>
   <xsl:variable name="tamus_new_requirement">Texas A&amp;M University System New Required Control</xsl:variable>
   <xsl:variable name="tamux_new_requirement">Texas A&amp;M University - Xample New Required Control</xsl:variable>

   <xsl:variable name="tx_required_by">Texas DIR Required By</xsl:variable>
   <xsl:variable name="tamus_required_by">Texas A&amp;M University System Required By</xsl:variable>
   <xsl:variable name="tamux_required_by">Texas A&amp;M University - Xample Required By</xsl:variable>
   ...
   <xsl:template match="control">
      <xsl:if test="prop[@name='tx_required_by' or @name='tamus_required_by' or @name='tamux_required_by']">
         ...
         <div class="control{ $withdrawn[boolean(.)] ! ' withdrawn' }">
            ...
            <details>
               ...
               <xsl:variable name="implementationLevels" select="prop[@name='implementation-level']/@value"/>

               <xsl:if test="$implementationLevels">
                  <div class="part implementation-level">
                     <p><span class="inline-head">Implementation Level</span>
                     <xsl:text>: </xsl:text>
                        <xsl:for-each select="$implementationLevels">
                           <xsl:if test="position() gt 1">; </xsl:if>
                           <xsl:value-of select="concat(upper-case(substring(replace(., 'system', 'information System'), 1, 1)), substring(replace(., 'system', 'information System'), 2))"/>
                        </xsl:for-each>
                     </p>
                  </div>
               </xsl:if>
...
-->
                  ...
                  <xsl:if test="empty(parent::control)">
                     <xsl:call-template name="unconditional-listing">
                        <xsl:with-param name="things" select="child::control[prop[@name='tx_required_by' or @name='tamus_required_by' or @name='tamux_required_by']]"/>
...
-->
   ...
   <xsl:template match="control/prop[@name='implementation-level']"/>

   <xsl:template match="control/prop[@name='contributes-to-assurance']">
      <div class="part {@name}">
         <p><span class="inline-head">Contributes to Assurance</span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="if (@value) then ' Yes' else ''"/>
         </p>
      </div>
   </xsl:template>

   <xsl:template match="control/prop[@name='tx_baseline']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tx_baseline"/></span>
         <xsl:text>: </xsl:text>
   ...
   <xsl:template match="control/prop[@name='tx_privacy_baseline']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tx_privacy_baseline"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="if (@value) then ' Yes' else ''"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tx_new_requirement']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tx_new_requirement"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="if (@value) then ' Yes' else ''"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tx_required_by']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tx_required_by"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="@value"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tamus_baseline']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tamus_baseline"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="@value"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tamus_privacy_baseline']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tamus_privacy_baseline"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="if (@value) then ' Yes' else ''"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tamus_new_requirement']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tamus_new_requirement"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="if (@value) then ' Yes' else ''"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tamus_required_by']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tamus_required_by"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="@value"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tamux_baseline']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tamux_baseline"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="@value"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tamux_privacy_baseline']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tamux_privacy_baseline"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="if (@value) then ' Yes' else ''"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tamux_new_requirement']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tamux_new_requirement"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="if (@value) then ' Yes' else ''"/>
         </p>
      </div>
   ...
   <xsl:template match="control/prop[@name='tamux_required_by']">
      <div class="part {@name}">
         <p><span class="inline-head"><xsl:value-of select="$tamux_required_by"/></span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="@value"/>
         </p>
      </div>
   ...
   <xsl:template match="control/part[@name='tamux_implementation']" priority="2">
      <div class="part {@name}">
         <xsl:copy-of select="@id"/>
            <xsl:apply-templates/>
       </div>
   ...
   <xsl:template priority="3" match="control/part[@name='tx_implementation'][empty(child::p)]">
      <div class="part {@name}">
         <xsl:copy-of select="@id"/>
         <p><span class="inline-head"><xsl:value-of select="$tx_implementation"/></span>:</p>
         <xsl:apply-templates/>
      </div>
   </xsl:template>
   ...
   <xsl:template priority="3" match="control/part[@name='tamus_implementation'][empty(child::p)]">
      <div class="part {@name}">
         <xsl:copy-of select="@id"/>
         <p><span class="inline-head"><xsl:value-of select="$tamus_implementation"/></span>:</p>
         <xsl:apply-templates/>
      </div>
   </xsl:template>

   <xsl:template priority="3" match="control/part[@name='tamux_implementation'][empty(child::p)]">
      <div class="part {@name}">
         <xsl:copy-of select="@id"/>
         <p><span class="inline-head"><xsl:value-of select="$tamux_implementation"/></span>:</p>
         <xsl:apply-templates/>
      </div>
   </xsl:template>
...
<!-- Statements of withdrawn controls are picked up with the 'withdrawn' prop below. -->
   ...
   <xsl:template match="control/part[@name='tx_implementation']/p">
      <xsl:choose>
         <xsl:when test=". is ancestor::part[@name='tx_implementation']/child::p[1]">
            <p><span class="inline-head"><xsl:value-of select="$tx_implementation"/></span>
               <xsl:text>: </xsl:text>
               <xsl:apply-templates/></p>
         </xsl:when>
         <xsl:otherwise>
   ...
   <xsl:template match="control/part[@name='tamus_implementation']/p">
      <xsl:choose>
         <xsl:when test=". is ancestor::part[@name='tamus_implementation']/child::p[1]">
            <p><span class="inline-head"><xsl:value-of select="$tamus_implementation"/></span>
               <xsl:text>: </xsl:text>
               <xsl:apply-templates/></p>
         </xsl:when>
         <xsl:otherwise>
   ...
   <xsl:template match="control/part[@name='tamux_implementation']/p">
      <xsl:choose>
         <xsl:when test=". is ancestor::part[@name='tamux_implementation']/child::p[1]">
            <p><span class="inline-head"><xsl:value-of select="$tamux_implementation"/></span>
               <xsl:text>: </xsl:text>
               <xsl:apply-templates/></p>
         </xsl:when>
         <xsl:otherwise>
...
-->
   ...
   <xsl:template priority="3" match="control/part[@name='assessment-method']"/>
...
<!-- Now matching Rev 5 display -->
   ...
   <xsl:template match="part/link[@rel = ('moved-to', 'incorporated-into')]"/>
...
<!-- Don't want to display -->
   ...
   <xsl:template match="insert[@type='param']">
      ...
      <span class="insert">
         <xsl:choose>
            <xsl:when test="$param/constraint">
               <xsl:apply-templates select="$param/constraint[last()]"/>
            </xsl:when>
            <xsl:when test="$param/value">
               <xsl:apply-templates select="$param/value[1]" mode="list"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:apply-templates select="$param/(select|label)[1]" mode="list"/>
            </xsl:otherwise>
         </xsl:choose>
      </span>
   </xsl:template>
   
   <xsl:template match="param/constraint/description/p">
      <span class="value">
         <xsl:apply-templates/>
      </span>
   </xsl:template>

   <xsl:template match="param/constraint">
      <span class="unassigned">
         <xsl:text>[TAMUX Assignment: </xsl:text>
         <xsl:apply-templates select="description[1]/p"/>
         <xsl:text>]</xsl:text>
      </span>
   </xsl:template>

   <xsl:template match="param/value[1]" mode="list">
      <span class="value">
   ...
   <xsl:template match="param/label[1]" mode="list">
      <span class="unassigned">
   ...
   <xsl:template match="param/select[1]" mode="list">
