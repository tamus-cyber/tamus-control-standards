...
<!-- TAMUS ADDED CONTENT BELOW HERE -->
   ...
   <xsl:template priority="3" match="control/prop[@name='ot-baseline']">
      <xsl:copy-of select="@id"/>
      <p><span class="inline-head">OT Baseline</span>
         <xsl:text>: </xsl:text>
         <xsl:value-of select="@value"/>
      </p>
   </xsl:template>
   ...
   <xsl:template priority="2" match="part[@name='ot-guidance']" mode="title">
      <h4>OT Discussion</h4>
   </xsl:template>
